...
- [x] **Lifecycle app**: la riproduzione si ferma esplicitamente quando l'app va in background (`PlayerScreen` osserva `ON_STOP` del lifecycle, oggi non c'era nessun codice del genere); riaprendo l'app si torna direttamente sul Player con lo stesso contenuto e l'ultima posizione salvata (`UltimoPlayerRepository`) — da lì Indietro va alla Dashboard, senza ricostruire l'intero stack di navigazione. Dimenticato appena si esce dal Player normalmente, cosi' una riapertura successiva senza essere ripassati dal background non ripropone un contenuto gia' lasciato volontariamente.
