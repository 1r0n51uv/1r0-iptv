...
- [x] **Overlay controlli player**: overlay custom per TV al posto dei controlli di default di Media3 (ADR 0007). Sinistra/destra saltano sempre avanti/indietro di un passo fisso con un indicatore visivo; su' porta il focus sul pulsante "Prossimo episodio" (quando c'e' un Episodio in coda), centro conferma o alterna play/pausa. Risolve anche il player bloccato sull'ultimo frame a fine episodio: l'`AndroidView` ora ha un `update` che ri-aggancia il `PlayerView` al nuovo `ExoPlayer` ad ogni cambio episodio, invece di restare legato al primo.
- [ ] **Reset Visto granulare**: rimozione del Visto per singolo Episodio, per intera Stagione o per intera Serie, esposta con la stessa pressione lunga già usata sulle card altrove nell'app. Nessun reset globale "cancella tutto".
